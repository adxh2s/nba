...
import pytest
import yaml
from modules.utils import file_tools
...
def test_load_from_json_and_yaml(tmp_path: Path):
    ...
    # Test load_from_json
    content_json = file_tools.load_from_json(str(json_path))
    assert content_json == data_json

    # Test load_from_yaml
    content_yaml = file_tools.load_from_yaml(str(yaml_path))
    assert content_yaml == data_yaml

    # Test load_from avec extension json
    assert file_tools.load_from(str(json_path)) == data_json

    # Test load_from avec extension yaml
    assert file_tools.load_from(str(yaml_path)) == data_yaml
    ...
    with pytest.raises(ValueError):
        file_tools.load_from(str(bad_path))
...
def test_is_path_in_root(tmp_path: Path):
    ...
    assert file_tools.is_path_in_root(str(file_path), root) is True
    assert file_tools.is_path_in_root("/not/in/root", root) is False


def test_build_file_path():
    assert file_tools.build_file_path("dir", "file", "txt") == os.path.join(
        "dir", "file.txt"
    )
    assert file_tools.build_file_path("", "file", ".json") == "file.json"
    assert file_tools.build_file_path(None, "file", "csv") == "file.csv"
...
def test_build_project_tree_and_get_node_path(tmp_path: Path):
    ...
    tree = file_tools.build_project_tree(base_path, structure)
    assert "docs" in tree
    path = file_tools.get_node_path(tree, "docs", "reports", "html")
    expected_path = os.path.join(base_path, "docs", "reports", "html")
    assert path == expected_path

    # test key error
    with pytest.raises(KeyError):
        file_tools.get_node_path(tree, "docs", "missing")


def test_format_report_filename():
    template = "{date:%Y%m%d%H%M}_{input}_report.html"
    name = "mydata.csv"
    filename = file_tools.format_report_filename(template, name)
